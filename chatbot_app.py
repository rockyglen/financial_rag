...
# Example for EC2 running on port 80 (via Nginx):
...
st.markdown("Connected to Deployed FastAPI RAG Service on AWS EC2.")
...
def ask_rag_backend(prompt: str):
    ...
    with st.spinner("Searching and synthesizing answer..."):
        try:
            ...
            return {
                "answer": "Could not connect to the API. Is the EC2 instance and Nginx running?",
                "source_documents": [],
            }
        except Exception as e:
            return {
                "answer": f"An unexpected client error occurred: {e}",
                "source_documents": [],
            }
